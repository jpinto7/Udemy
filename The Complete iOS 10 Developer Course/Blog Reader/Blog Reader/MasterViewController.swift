...
class MasterViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if let url = URL(string: "https://www.googleapis.com/blogger/v3/blogs/10861780/posts?key=AIzaSyDcVAs-T0tgn2RC6FygNPwjZl8HFMRhMbc") {
            let task = URLSession.shared.dataTask(with: url) {
                data, response, error in
                if error != nil {
                    print(error ?? "URL processing failed")
                } else {
                    if let urlContent = data {
                        do {
                            ...
                            if let items = jsonData["items"] as? NSArray {
                                self.deleteEvents()
                                for item in items {
                                    self.insertNewObject(item as AnyObject)
                                }
    ...
    func insertNewObject(_ item: AnyObject) {
